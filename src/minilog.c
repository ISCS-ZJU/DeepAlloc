...
static inline void *minilog_file_create(size_t align, unsigned numa_ind)
{
    void *addr, *tmp;
    ...
    int is_pmem;
    if (numa_ind)
        sprintf(str, "/mnt/pmem/deepalloc_files/minilog_file");
    else
        sprintf(str, "/mnt/pmem0.1/deepalloc_files/minilog_file");
...
{
    minilog_t *log = (minilog_t *)((uint64_t)minilog_file_create(8 * 1024, numa_ind) + 4 * 1024 - MINILOG_SIZE / 2);
