...
void vslab_init(vslab_t *vslab, size_t sc_idx)
{
    ...
    vslab->rbitmap = (bitmap_t *)_malloc(sc.n_bitmaps * sizeof(bitmap_t));
    memset(vslab->rbitmap, 0xffU, sc.n_bitmaps * sizeof(bitmap_t));

    ql_elm_new(vslab, numa_link);
...
int slab_pop_one_cache(arena_t *arena, cache_t *cache, vslab_t *vslab, bin_t *bin, size_t sc_idx)
{
    ...
    if (vslab->bitmap_header.nbits == sc.nbits)
    {
        if (vslab->r_dirty)
        {
            if (pthread_mutex_trylock(&arena->numa_lock))
                return -1;
            ...
            vslab->r_dirty = 0;
            _free(temp_meta);
            ql_remove(&arena->numa_list, vslab, numa_link);
...

{
    ...
    if (extent->arena != free_arena)
    {
        ...
        arena = extent->arena;
        if (arena->numa_ind != free_arena->numa_ind)
        {
            int index = (((uintptr_t)ptr - vslab->sc.roffset - (uintptr_t)vslab->slab)) / vslab->sc.bitsize;
            numa_log_alloc(&free_tcache->slab_log, (uintptr_t)ptr, arena);
            pthread_mutex_lock(&vslab->slab_lock);
            bitmap_set_bits(vslab->rbitmap, index);
            uint8_t flag = 0;
            if (!vslab->r_dirty)
                vslab->r_dirty = flag = 1;
            pthread_mutex_unlock(&vslab->slab_lock);
            if (flag)
            {
                pthread_mutex_lock(&arena->numa_lock);
                ql_tail_insert(&arena->numa_list, vslab, numa_link);
                pthread_mutex_unlock(&arena->numa_lock);
            }
            return;
        }
...
#endif
            if (vslab->bitmap_header.nbits == 0 && !vslab->r_dirty)
            {
                pthread_mutex_unlock(&vslab->slab_lock);
