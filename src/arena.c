#define DEEPALLOC_ARENA_C_
#include "deepalloc/internal/deepalloc_internal.h"

/******************************************************************************/
/* Data. */

const char PMEMPATH[2][50] = {"/mnt/pmem0.1/deepalloc_files/deepalloc_files_", "/mnt/pmem/deepalloc_files/deepalloc_files_"};
...
int arena_get_new_memory_locked(arena_t *arena, size_t npages)
{
	...
	arena->memory_new = arena_file_map_locked(arena, CHUNK_SIZE, PMEMPATH[arena->numa_ind], CHUNK_SIZE);
...
bool arena_new(arena_t *arena, unsigned ind)
{
	...
	pthread_mutex_init(&arena->numa_lock, 0);

	ql_new(&arena->numa_list);

	int cpu_id;
	getcpu(&cpu_id, &arena->numa_ind, NULL);

	if (sem_init(&arena->slab_sem, 1, 0))
	{
		printf("semaphore fail! %d\n", arena->ind);
		exit(1);
	}
	sems[0][arena->ind] = &arena->slab_sem;
	if (sem_init(&arena->extent_sem, 1, 0))
	{
		printf("semaphore fail! %d\n", arena->ind);
		exit(1);
	}
	sems[1][arena->ind] = &arena->extent_sem;
    if (sem_init(&arena->GC_sem, 1, 0))
	{
		printf("semaphore fail! %d\n", arena->ind);
		exit(1);
	}
				  ...
				  szind_t szind, bool slab)
{
	...
	arena_t *arena = extent->arena, *free_arena = choose_arena(NULL);
	uint64_t log_ptr = (uint64_t)extent->log;
	arena_extents_dirty_dalloc(tcache, extent->arena, extent);
	pthread_mutex_lock(&arena->log_lock);
    if(free_arena->numa_ind != arena->numa_ind)
        add_tomb(arena, arena->log, &arena->shadow_tomb, log_ptr);
    else
    	add_log(arena, &arena->log, FREE_LOG, log_ptr, (uint64_t)NULL, false);
	pthread_mutex_unlock(&arena->log_lock);
}
