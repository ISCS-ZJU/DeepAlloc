...
bool rtree_new(rtree_t *rtree, bool zeroed)
{
	...
	if (pthread_mutex_init(&rtree->init_lock, 0))
		return true;
...
rtree_node_alloc_impl(rtree_t *rtree, size_t nelms)
{
	/* return (rtree_node_elm_t *)base_alloc(tsdn, b0get(), nelms * sizeof(rtree_node_elm_t), CACHELINE); */
	void* ret = NULL;
	int e = posix_memalign(&ret, CACHELINE_SIZE, nelms * sizeof(rtree_node_elm_t));
	memset(ret,0x0,nelms * sizeof(rtree_node_elm_t));
	assert(!e);
	return (rtree_node_elm_t *)ret;
}
rtree_node_alloc_t *JET_MUTABLE rtree_node_alloc = rtree_node_alloc_impl;

static void
rtree_node_dalloc_impl(rtree_t *rtree, rtree_node_elm_t *node)
{
...
rtree_leaf_alloc_impl(rtree_t *rtree, size_t nelms)
{
	//return (rtree_leaf_elm_t *)base_alloc(tsdn, b0get(), nelms * sizeof(rtree_leaf_elm_t), CACHELINE);
	void* ret = NULL;
	int e = posix_memalign(&ret, CACHELINE_SIZE, nelms * sizeof(rtree_leaf_elm_t));
	memset(ret,0x0,nelms * sizeof(rtree_leaf_elm_t));
	assert(!e);
	return (rtree_leaf_elm_t *)ret;
}
rtree_leaf_alloc_t *JET_MUTABLE rtree_leaf_alloc = rtree_leaf_alloc_impl;

static void
rtree_leaf_dalloc_impl(rtree_t *rtree, rtree_leaf_elm_t *leaf)
{
...
static rtree_node_elm_t *
rtree_node_init(rtree_t *rtree, unsigned level,
				atomic_p_t *elmp)
{

	pthread_mutex_lock(&rtree->init_lock);
	/*
	...
	if (node == NULL)
	{
		node = rtree_node_alloc(rtree, ZU(1) << rtree_levels[level].bits);
		if (node == NULL)
		{
			pthread_mutex_unlock(&rtree->init_lock);
			return NULL;
		}
		/*
...
static rtree_leaf_elm_t *
rtree_leaf_init(rtree_t *rtree, atomic_p_t *elmp)
{
	pthread_mutex_lock(&rtree->init_lock);
	/*
	 * If *elmp is non-null, then it was initialized with the init lock
	...
	if (leaf == NULL)
	{
		leaf = rtree_leaf_alloc(rtree, ZU(1) << rtree_levels[RTREE_HEIGHT - 1].bits);
		if (leaf == NULL)
		{
			pthread_mutex_unlock(&rtree->init_lock);
			return NULL;
...
static rtree_node_elm_t *
rtree_child_node_read(rtree_t *rtree, rtree_node_elm_t *elm,
					  unsigned level, bool dependent)
{
	...
	{
		node = rtree_node_init(rtree, level + 1, &elm->child);
	}
	assert(!dependent || node != NULL);
	return node;
}
...
static rtree_leaf_elm_t *
rtree_child_leaf_read(rtree_t *rtree, rtree_node_elm_t *elm,
					  unsigned level, bool dependent)
{
	rtree_leaf_elm_t *leaf;
	...
	{
		leaf = rtree_leaf_init(rtree, &elm->child);
	}
	assert(!dependent || leaf != NULL);
	return leaf;
}
...
rtree_leaf_elm_t *
rtree_leaf_elm_lookup_hard(rtree_t *rtree, rtree_ctx_t *rtree_ctx,
...
#define RTREE_GET_CHILD(level)                                                           \
	{                                                                                    \
		...
		if (level + 2 < RTREE_HEIGHT)                                                    \
		{                                                                                \
			node = init_missing ? rtree_child_node_read(rtree,                     \
		...
		{                                                                                \
			leaf = init_missing ? rtree_child_leaf_read(rtree,                     \
														&node[subkey], level, dependent) \
