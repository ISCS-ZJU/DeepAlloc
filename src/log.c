...
void *log_file_create(size_t align, arena_t *arena)
{
    void *addr, *tmp;
    ...
    int is_pmem;
    if (arena->numa_ind)
        sprintf(str, "%slog_%ld", "/mnt/pmem/deepalloc_files/deepalloc_files_", arena->ind);
    else
        sprintf(str, "%slog_%ld", "/mnt/pmem0.1/deepalloc_files/deepalloc_files_", arena->ind);
...
vlog_t *log_create(arena_t *arena)
{
    ...
    vlog->log_file_addr = log_file_create(LCHUNK_SIZE, arena);
    vlog->log_file_used = LCHUNK_SIZE;
    log_file_head_t *log_file_head = (log_file_head_t *)vlog->log_file_addr;
    log_file_head->alt = 0;
    persist(&log_file_head->alt, sizeof(log_file_head->alt));
    log_file_head->timestamp = 0;
    persist(&log_file_head->timestamp, sizeof(log_file_head->timestamp));
...
int get_heap_usage()
{
    ...
    get_filepath(arena->ind, fname, arena->file_id, PMEMPATH[arena->numa_ind]);
...
void slow_GC(arena_t *arena)
{
    ...
    while (1)
    {
        ...
        if(vlog->num_of_chunks * N_ITEMS_PER_LCHUNK / vlog->num_of_items >= 10)
        {
                        ...
                        if (cursor_vlchunk->bitmap[i] & (1ULL << j))
                        {
                            ...
                            if (cursor_item->type == NORMAL_LOG)
                            {
                                ...
                                if (cursor_item->size_version_id & 1ULL)
                                {
                                    vslab_t *vslab;
                                RETRY:
                                    vslab = (vslab_t *)rtree_extent_read(&extents_rtree, &rtree_ctx, cursor_item->ptr, true);
                                    extent = vslab->eslab;
                                    if (extent == 0)
                                    {
                                        goto RETRY;
                                ...
                                {
                                    extent = rtree_extent_read(&extents_rtree, &rtree_ctx, cursor_item->ptr, false);
                                }
            ...
            persist(&log_file_head->alt, sizeof(log_file_head->alt));
            log_file_head->timestamp = numa_timestamp();
            persist_one(&log_file_head->timestamp);
            shadow_chunk_t* ptr = arena->shadow_tomb->next;
            while(ptr)
            {
                shadow_chunk_t* p = ptr->next;
                _free(ptr);
                ptr = p;
            }
...
void add_tomb(arena_t *arena, vlog_t *vlog, shadow_chunk_t** tombs, uint64_t ptr)
{
    if((*tombs)->cnt == SHADOW_CHUNK_SIZE)
    {
        shadow_chunk_t* p = (shadow_chunk_t*)_malloc(sizeof(shadow_chunk_t));
        p->cnt = 0;
        p->next = *tombs;
        *tombs = p;
    }

    log_item_t *item = (*tombs)->items + ((*tombs)->cnt++);

    lchunk_t *lchunk = ptr2lchunk(ptr);
    int index = ptr2lindex(ptr, lchunk);
    ...
    if (!vlchunk)
    {
        printf("tomb &lchunk = %p,vlchunk= %p\n", lchunk, vlchunk);
        assert(vlchunk);
        exit(0);
    }
    ...
    numa_log_alloc(&tcache->extent_log, (item->type << 62) + (item->ptr << 14) + item->size_version_id, arena);
}
